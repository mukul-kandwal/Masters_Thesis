...
def XGBRegressor_script(data_set, test_seasons, Hyperparams=Default_Hyperparams, batch_size=100):
    # Generate all possible pairs from the dataset
    # pairs = list(combinations(data_set, test_num))
    # net_score = 0
    ...
    # Prepare the training data for the XGBoost model
    X, y = prepare_panel_data(training_set, Hyperparams["max_length"], Hyperparams["pad_value"])
    
    # Initialize and train the XGBoost model using GPU
    model = XGBRegressor(n_estimators=Hyperparams["n_estimators"], learning_rate=Hyperparams["learning_rate"], random_state=Hyperparams["random_state"], max_depth=Hyperparams["max_depth"],
                         subsample=Hyperparams["subsample"], colsample_bytree=Hyperparams["colsample_bytree"], gamma=Hyperparams["gamma"], reg_alpha=Hyperparams["reg_alpha"], reg_lambda=Hyperparams["reg_lambda"],
                         tree_method = "hist", device = "cuda", max_bin=Hyperparams["max_bin"], predictor='gpu_predictor')
    model.fit(X, y)
    ...
    for x_test in X_test:
        prediction_values.append(float(model.predict(np.array([x_test]))[0]))
...
def Run_XGBRegressor(data_set, test_num, Hyperparams=Default_Hyperparams, batch_size=100):
    ...
    for training_season in training_season_sets:
      net_score += XGBRegressor_script(data_set, training_season, Hyperparams)
      
    return net_score/len(training_season_sets)

def grid_search_for_XGB(param_grid, data_set):
...
if len(sys.argv) > 6:
    ...
    for i, row in df.iterrows():
        ...
        grid_search = {
        name: [
            int(row[f"metadata_{name}"]) if float(row[f"metadata_{name}"]).is_integer()
            else float(row[f"metadata_{name}"])
        ]
        ...
        grid_search_for_XGB(grid_search, FullAgnostic_Tseries)
