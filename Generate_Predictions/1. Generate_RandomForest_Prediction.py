...
def RandomForest_script(data_set, test_seasons, Hyperparams):
    ...
    prediction_values = [float(model.predict(np.array([x_test]))[0]) for x_test in X_test]  # Start with an initial prediction value
...
def predict_RandomForest(data_set, test_num, Hyperparams):
    ...
    for training_season in training_season_sets:
      net_score += RandomForest_script(data_set, training_season, Hyperparams)
